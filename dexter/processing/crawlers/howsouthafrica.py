...
class HowSouthAfricaCrawler(BaseCrawler):
    HSA_RE = re.compile('(www\.)?howsouthafrica.com')
    ...
    def offer(self, url):
        ...
        parts = urlparse(url)
        return bool(self.HSA_RE.match(parts.netloc))
    ...
    def extract(self, doc, raw_html):
        ...
        super(HowSouthAfricaCrawler, self).extract(doc, raw_html)
        ...
        nodes = soup.select("#content .entry-essentials .entry-content p")
        if len(nodes) > 1:
        ...
        # gather author
        author = self.extract_plaintext(soup.select(".entry-author[itemprop='author'] [itemprop='name']"))
